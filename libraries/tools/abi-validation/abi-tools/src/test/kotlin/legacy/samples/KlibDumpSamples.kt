...
class KlibDumpSamples {
    ...
    fun mergeDumps() {
        ...
        val mergedDump = AbiTools.legacy.emptyKlibDump().apply {
            merge(linuxX64Dump)
            merge(linuxArm64Dump)
        }
        val mergedDumpContent = buildString { mergedDump.printTo(this) }
    ...
    fun mergeDumpObjects() {
        ...
        mergedDump.merge(AbiTools.legacy.parseKlibDump(linuxArm64Dump))
        mergedDump.merge(AbiTools.legacy.parseKlibDump(linuxX64Dump).also { it.setCustomName("linuxX86_64") })
        val mergedDumpContent = buildString { mergedDump.printTo(this) }
        ...
        val filteredDumpContent = buildString { mergedDump.printTo(this) }
        assertEquals("""
            // Klib ABI Dump
            // Targets: [linuxArm64]
            // Rendering settings:
    ...
    fun updateMergedDump() {
        ...
        mergedDump.replace(AbiTools.legacy.parseKlibDump(newLinuxX64Dump).also { it.setCustomName("linuxX86_64") })

        val mergedDumpContent = mergedDump.printTo(StringBuilder()).toString()
        assertEquals("""
            // Klib ABI Dump
            // Targets: [linuxArm64, linuxX64.linuxX86_64]
            // Rendering settings:
    ...
    fun extractTargets() {
        ...
        val dump = AbiTools.legacy.parseKlibDump(mergedDump)
        assertEquals(
        ...
        val filteredDumpContent = buildString { dump.printTo(this) }
        assertEquals("""
            // Klib ABI Dump
            // Targets: [linuxArm64, linuxX64]
            // Rendering settings:
    ...
    fun inferDump() {
        ...
        val inferredIosArm64Dump = inferAbi(
            ...
            supportedTargetDumps = listOf(AbiTools.legacy.parseKlibDump(linuxDump)),
            oldMergedDump = AbiTools.legacy.parseKlibDump(oldMergedDump))
