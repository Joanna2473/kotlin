...
class KlibDumpTest {
    ...
    fun emptyDump() {
        val dump = buildString {
            AbiTools.klib.emptyDump().printTo(this)
        }
        assertEquals("", dump)

        assertFailsWith<IllegalArgumentException> {
            AbiTools.klib.parse(tmpFolder.newFile())
        }
    }
    ...
    fun loadFromNonExistingFile() {
        assertFailsWith<FileNotFoundException> {
            AbiTools.klib.parse(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.klib.emptyDump().merge(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.klib.parseKlib(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.klib.emptyDump().mergeFromKlib(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
    }

    @Test
    fun loadKlibFromNonKlib() {
        assertFailsWith<IllegalStateException> { AbiTools.klib.parseKlib(tmpFolder.root) }
        assertFailsWith<IllegalStateException> { AbiTools.klib.parseKlib(tmpFolder.newFile()) }

        assertFailsWith<IllegalStateException> { AbiTools.klib.emptyDump().also { it.mergeFromKlib(tmpFolder.root) } }
        assertFailsWith<IllegalStateException> { AbiTools.klib.emptyDump().also { it.mergeFromKlib(tmpFolder.newFile()) } }
    }

    @Test
    fun loadFromDirectory() {
        assertFailsWith<IllegalArgumentException> {
            AbiTools.klib.parse(tmpFolder.root)
        }
        assertFailsWith<IllegalArgumentException> {
            AbiTools.klib.emptyDump().merge(tmpFolder.root)
        }
    }

    @Test
    fun loadDumpWithSingleTarget() {
        val klibDump = AbiTools.klib.parse(asFile(rawLinuxDump))
        ...
        val mergedKlibDump = AbiTools.klib.parse(asFile(mergedLinuxDump))
    ...
    fun mergeDumpWithSingleTarget() {
        val klibDump = AbiTools.klib.emptyDump().also { it.merge(asFile(rawLinuxDump)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64")), klibDump.targets)
        assertEquals(mergedLinuxDump, buildString { klibDump.printTo(this) })

        val mergedKlibDump = AbiTools.klib.emptyDump().also { it.merge(asFile(mergedLinuxDump)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDump, buildString { mergedKlibDump.printTo(this) })
    }
    ...
    fun loadDumpWithSingleTargetWithCustomName() {
        ...
        val customTargetDump = AbiTools.klib.parse(asFile(mergedLinuxDumpWithCustomName))
    ...
    fun mergeDumpWithSingleTargetWithCustomName() {
        ...
        val mergedKlibDump =
            AbiTools.klib.emptyDump().also { it.merge(asFile(mergedLinuxDump)) }
        mergedKlibDump.setCustomName("testTarget")
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { mergedKlibDump.printTo(this) })

        val customTargetDump = AbiTools.klib.emptyDump().also { it.merge(asFile(mergedLinuxDumpWithCustomName)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), customTargetDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { customTargetDump.printTo(this) })
    }
    ...
    fun loadMultitargetDump() {
        val dump = AbiTools.klib.parse(asFile(rawMultitargetDump))
        assertEquals(21, dump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { dump.printTo(this) })

        val mergedDump = AbiTools.klib.parse(asFile(mergedMultitargetDump))
    ...
    fun mergeMultitargetDump() {
        val dump = AbiTools.klib.emptyDump().also { it.merge(asFile(rawMultitargetDump)) }
        assertEquals(21, dump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { dump.printTo(this) })

        val mergedDump = AbiTools.klib.emptyDump().also { it.merge(asFile(mergedMultitargetDump)) }
        assertEquals(21, mergedDump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { mergedDump.printTo(this) })
    }
    ...
    @Test
    fun retainAll() {
        val dump = AbiTools.klib.parse(asFile(mergedMultitargetDump))
    ...
    fun removeDeclarationsAlongWithTargets() {
        val dump = AbiTools.klib.parse(asFile(mergedLinuxDumpWithTargetSpecificDeclaration))
    ...
    @Test
    fun testMergeDumps() {
        val dump = AbiTools.klib.emptyDump().also {
    ...
    fun mergeDumpsWithIntersectingTargets() {
        val mergedDump = AbiTools.klib.parse(asFile(rawMultitargetDump))
    ...
    fun inferWithoutAnOldDump() {
        ...
        val linuxDump = AbiTools.klib.parse(
            asFile(
                """
                // Klib ABI Dump
                // Targets: [linuxArm64]
    ...
    fun inferFromAnOldDumpOnly() {
        ...
        val oldDump = AbiTools.klib.parse(
            asFile(
                """
            // Klib ABI Dump
    ...
    @Test
    fun inferFromSelf() {
        val dump = AbiTools.klib.parse(asFile(mergedLinuxDump))
        assertFailsWith<IllegalArgumentException> {
    ...
    fun inferFromIntersectingDumps() {
        assertFailsWith<IllegalArgumentException> {
            inferAbi(
                ...
                listOf(
                    AbiTools.klib.parse(asFile(mergedLinuxDump)),
                    AbiTools.klib.parse(asFile(mergedMultitargetDump))
                )
            )
        }
    }
