...
class KlibDumpTest {
    ...
    fun emptyDump() {
        val dump = buildString {
            AbiTools.legacy.emptyKlibDump().printTo(this)
        }
        assertEquals("", dump)

        assertFailsWith<IllegalArgumentException> {
            AbiTools.legacy.parseKlibDump(tmpFolder.newFile())
        }
    }
    ...
    fun loadFromNonExistingFile() {
        assertFailsWith<FileNotFoundException> {
            AbiTools.legacy.parseKlibDump(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.legacy.emptyKlibDump().merge(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.legacy.parseKlib(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
        assertFailsWith<FileNotFoundException> {
            AbiTools.legacy.emptyKlibDump().mergeFromKlib(tmpFolder.root.resolve(UUID.randomUUID().toString()))
        }
    }

    @Test
    fun loadKlibFromNonKlib() {
        assertFailsWith<IllegalStateException> { AbiTools.legacy.parseKlib(tmpFolder.root) }
        assertFailsWith<IllegalStateException> { AbiTools.legacy.parseKlib(tmpFolder.newFile()) }

        assertFailsWith<IllegalStateException> { AbiTools.legacy.emptyKlibDump().also { it.mergeFromKlib(tmpFolder.root) } }
        assertFailsWith<IllegalStateException> { AbiTools.legacy.emptyKlibDump().also { it.mergeFromKlib(tmpFolder.newFile()) } }
    }

    @Test
    fun loadFromDirectory() {
        assertFailsWith<IllegalArgumentException> {
            AbiTools.legacy.parseKlibDump(tmpFolder.root)
        }
        assertFailsWith<IllegalArgumentException> {
            AbiTools.legacy.emptyKlibDump().merge(tmpFolder.root)
        }
    }

    @Test
    fun loadDumpWithSingleTarget() {
        val klibDump = AbiTools.legacy.parseKlibDump(asFile(rawLinuxDump))
        assertEquals(setOf(KlibTarget.parse("linuxX64")), klibDump.targets)
        assertEquals(mergedLinuxDump, buildString { klibDump.printTo(this) })

        val mergedKlibDump = AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDump))
        assertEquals(setOf(KlibTarget.parse("linuxX64")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDump, buildString { mergedKlibDump.printTo(this) })
    }

    @Test
    fun mergeDumpWithSingleTarget() {
        val klibDump = AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(rawLinuxDump)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64")), klibDump.targets)
        assertEquals(mergedLinuxDump, buildString { klibDump.printTo(this) })

        val mergedKlibDump = AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(mergedLinuxDump)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDump, buildString { mergedKlibDump.printTo(this) })
    }
    ...
    fun loadDumpWithSingleTargetWithCustomName() {
        ...
        assertEquals(mergedLinuxDumpWithCustomName, buildString { klibDump.printTo(this) })

        val mergedKlibDump = AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDump))
        mergedKlibDump.setCustomName("testTarget")
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { mergedKlibDump.printTo(this) })

        val customTargetDump = AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDumpWithCustomName))
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), customTargetDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { customTargetDump.printTo(this) })
    }
    ...
    fun mergeDumpWithSingleTargetWithCustomName() {
        ...
        val mergedKlibDump =
            AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(mergedLinuxDump)) }
        mergedKlibDump.setCustomName("testTarget")
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), mergedKlibDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { mergedKlibDump.printTo(this) })

        val customTargetDump = AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(mergedLinuxDumpWithCustomName)) }
        assertEquals(setOf(KlibTarget.parse("linuxX64.testTarget")), customTargetDump.targets)
        assertEquals(mergedLinuxDumpWithCustomName, buildString { customTargetDump.printTo(this) })
    }
    ...
    fun loadMultitargetDump() {
        val dump = AbiTools.legacy.parseKlibDump(asFile(rawMultitargetDump))
        assertEquals(21, dump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { dump.printTo(this) })

        val mergedDump = AbiTools.legacy.parseKlibDump(asFile(mergedMultitargetDump))
        assertEquals(21, mergedDump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { mergedDump.printTo(this) })
    }

    @Test
    fun mergeMultitargetDump() {
        val dump = AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(rawMultitargetDump)) }
        assertEquals(21, dump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { dump.printTo(this) })

        val mergedDump = AbiTools.legacy.emptyKlibDump().also { it.merge(asFile(mergedMultitargetDump)) }
        assertEquals(21, mergedDump.targets.size)
        assertEquals(mergedMultitargetDump, buildString { mergedDump.printTo(this) })
    }
    ...
    @Test
    fun retainAll() {
        val dump = AbiTools.legacy.parseKlibDump(asFile(mergedMultitargetDump))
    ...
    fun retainSingle() {
        ...
        assertEquals(mergedMultitargetDumpFiltered, buildString { dump.printTo(this) })
    }
    ...
    fun retainNone() {
        ...
        assertEquals("", buildString { dump.printTo(this) })
    }
    ...
    fun removeDeclarationsAlongWithTargets() {
        val dump = AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDumpWithTargetSpecificDeclaration))
        val toRemove = KlibTarget.parse("linuxArm64")

        dump.remove(listOf(toRemove))
        assertEquals(mergedLinuxDump, buildString { dump.printTo(this) })
    }
    ...
    fun testCopy() {
        ...
        assertEquals(mergedLinuxDumpWithTargetSpecificDeclaration, buildString { copy.printTo(this) })
    }

    @Test
    fun testMergeDumps() {
        val dump = AbiTools.legacy.emptyKlibDump().also {
            it.merge(asFile(mergedLinuxDump))
            it.merge(asFile(mergedLinuxArm64Dump))
        }
        assertEquals(mergedLinuxDumpWithTargetSpecificDeclaration, buildString { dump.printTo(this) })
    }

    @Test
    fun mergeDumpsWithIntersectingTargets() {
        val mergedDump = AbiTools.legacy.parseKlibDump(asFile(rawMultitargetDump))
    ...
    fun inferWithoutAnOldDump() {
        ...
        val linuxDump = AbiTools.legacy.parseKlibDump(
            asFile(
                """
                // Klib ABI Dump
                // Targets: [linuxArm64]
        ...
        val inferredDumpContent = buildString { inferredIosArm64Dump.printTo(this) }
        assertEquals(
            """
                // Klib ABI Dump
                // Targets: [iosArm64]
    ...
    fun inferFromAnOldDumpOnly() {
        ...
        val oldDump = AbiTools.legacy.parseKlibDump(
            asFile(
                """
            // Klib ABI Dump
    ...
    @Test
    fun inferFromSelf() {
        val dump = AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDump))
        assertFailsWith<IllegalArgumentException> {
    ...
    fun inferFromIntersectingDumps() {
        assertFailsWith<IllegalArgumentException> {
            inferAbi(
                ...
                listOf(
                    AbiTools.legacy.parseKlibDump(asFile(mergedLinuxDump)),
                    AbiTools.legacy.parseKlibDump(asFile(mergedMultitargetDump))
                )
            )
        }
    }
    ...
    fun iterativeGrouping() {
        val dump = AbiTools.legacy.parseKlibDump(
            asFile(
                """
            // Klib ABI Dump
        ...
        """.trimIndent()
        assertEquals(expectedDump, buildString { dump.printTo(this) })
    }
    ...
    fun saveToFile() {
        ...
        dump.printToFile(tempFile)

        assertEquals(
            buildString { dump.printTo(this) },
