...
class CasesPublicAPITest {
    ...
    private fun snapshotAPIAndCompare(testClassRelativePath: String, vararg excludeAnnotatedWith: String) {
        ...
        val testClasses = testClassPaths.flatMap { it.toPath().walk().map(Path::toFile) }.filter { it.name.endsWith(".class") }
        ...
        if (!target.exists()) {
            target.bufferedWriter().use { writer ->
                AbiTools.legacy.dumpJvm(writer, testClasses, filters)
            }
            fail("Expected data file did not exist. Generating: $target")
        } else {
            val stringBuffer = StringBuffer()
            AbiTools.legacy.dumpJvm(stringBuffer, testClasses, filters)
