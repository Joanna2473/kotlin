...
internal fun compileMatcher(filters: AbiFilters): FiltersMatcher {
    val included = ComplexMatcher(filters.includedClasses.processClasses(), filters.includedAnnotatedWith)
    val excluded = ComplexMatcher(filters.excludedClasses.processClasses(), filters.excludedAnnotatedWith)
    return FiltersMatcherImpl(included, excluded)
}

internal fun emptyMatcher(): FiltersMatcher {
    return FiltersMatcherImpl.EMPTY
}
...
private class FiltersMatcherImpl(private val include: ComplexMatcher, private val exclude: ComplexMatcher) : FiltersMatcher {
    ...
    override fun testClass(className: String, annotations: List<String>): FilterResult {
        ...
        if (hasAnnotationFilters) {
            val checkExclude = exclude.annotations.isNotEmpty()
            val checkInclude = include.annotations.isNotEmpty()

            annotations.forEach { annotation ->
                if (checkExclude && exclude.annotations.contains(annotation)) return FilterResult.EXCLUDED

                if (checkInclude && !matchInclude && include.annotations.contains(annotation)) {
                    matchInclude = true
                }
    ...
    override fun isExcludedByAnnotations(annotationNames: List<String>): Boolean {
        val checkExclude = exclude.annotations.isNotEmpty()
        var checkInclude = include.annotations.isNotEmpty()

        var inInclude = false
        annotationNames.forEach { annotation ->
            if (checkExclude && exclude.annotations.contains(annotation)) return true
            if (checkInclude && include.annotations.contains(annotation)) {
                checkInclude = false
                inInclude = true
        ...
        return include.annotations.isNotEmpty() && !inInclude
    }
    ...
    companion object {
        val EMPTY =
            FiltersMatcherImpl(ComplexMatcher(WildcardsMatcher.EMPTY, emptySet()), ComplexMatcher(WildcardsMatcher.EMPTY, emptySet()))
    }
}

private class ComplexMatcher(val classNames: WildcardsMatcher, val annotations: Set<String>)
...
private fun Iterable<String>.processClasses(): WildcardsMatcher {
    ...
    forEach { classFilter ->
        val hasQuestionWildcard = classFilter.indexOf('?') > -1
