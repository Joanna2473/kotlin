...
internal class KlibDumpImpl : KlibDump {
    ...
    override fun partialMerge(other: KlibDump, targetsFromOther: List<KlibTarget>) {
        val dumpsFromOther = targetsFromOther.map { targetFromOther ->
            ...
            val supportedTargetDumps = if (matchingTargets.isNotEmpty()) {
                copy().also { it.retain(matchingTargets) }
            } else {
                val targetFromReference = other.targets.firstOrNull { it.targetName == targetFromOther.targetName }
                return@map if (targetFromReference != null) {
                    val copy = other.copy()
                    copy.retain(listOf(targetFromReference))
                    copy
                } else if (other.targets.isNotEmpty()) {
                    KlibDumpImpl()
                } else {
                    throw IllegalStateException(
                        "The target $targetFromOther is not supported by the host compiler " +
                                "and there are no targets similar to $targetFromOther to infer a dump from it."
                    )
                }
            }
...
internal fun KlibDump.mergeFromKlib(
    ...
    this.merge(AbiTools.legacy.parseKlib(klibFile, filters))
}
