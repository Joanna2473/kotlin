...
class PowerAssertCallTransformer(
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        ...
        if (delegate.function.parameters.size == function.parameters.size) {
            messageArgument = expression.arguments[expression.arguments.size - 1]
            roots = (0 until expression.arguments.size - 1)
                .map { index -> expression.arguments[index] }
                .map { arg -> arg?.let { buildTree(it, expression) } }
        } else {
            messageArgument = null
            roots = (0 until expression.arguments.size)
                .map { index -> expression.arguments[index] }
                .map { arg -> arg?.let { buildTree(it, expression) } }
        }
    ...
    private fun findDelegates(function: IrFunction): List<FunctionDelegate> {
        ...
        return possible.mapNotNull { overload ->
            ...
            for (i in values.indices) {
                ...
                when (value.kind) {
                    // Value parameters may only be assignable.
