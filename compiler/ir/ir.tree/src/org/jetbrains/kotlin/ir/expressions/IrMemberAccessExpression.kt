...
abstract class IrMemberAccessExpression<S : IrSymbol> : IrDeclarationReference() {
    ...
    private data class ExplicitShapeInitialization(
        val trace: List<StackTraceElement>,
        val hasDispatchReceiver: Boolean,
        val hasExtensionReceiver: Boolean,
    )
    private var _lastShapeInit: ExplicitShapeInitialization? = null
    ...
    internal fun initializeTargetShapeExplicitly(
        ...
        if (isFromTargetUpdate) {
            ...
            if (symbol.isBound) {
                (symbol.owner as? IrFunction)?.let { target ->
                    require(hasDispatchReceiver == (target.dispatchReceiverParameter != null))
                    require(hasExtensionReceiver == (target.extensionReceiverParameter != null))
                }
            }
        }

        _lastShapeInit = ExplicitShapeInitialization(
            StackWalker.getInstance().walk { stack ->
                stack.skip(2).limit(3)
    ...
    private fun updateTargetShape() {
        if (!symbol.isBound) {
            return
        }
        val target = symbol.owner as? IrFunction
            ?: return

        val newHasDispatchReceiver = target.dispatchReceiverParameter != null
        val newHasExtensionReceiver = target.extensionReceiverParameter != null

        updateReceiverChange(0, targetHasDispatchReceiver, newHasDispatchReceiver)
        targetHasDispatchReceiver = newHasDispatchReceiver
        updateReceiverChange(getExtensionReceiverIndex(), targetHasExtensionReceiver, newHasExtensionReceiver)
        targetHasExtensionReceiver = newHasExtensionReceiver
    }

    private fun updateReceiverChange(index: Int, hadBefore: Boolean, hasNow: Boolean) {
        if (!hadBefore && hasNow) {
            arguments.add(index, null)
        } else if (hadBefore && !hasNow) {
            require(arguments[index] == null) { "Receiver argument must be removed before receiver parameter is removed" }
    ...
    var dispatchReceiver: IrExpression?
        ...
        set(value) {
            updateTargetShape()
            setReceiverArgument(0, value, targetHasDispatchReceiver)
        }
    ...
    var extensionReceiver: IrExpression?
        ...
        set(value) {
            updateTargetShape()
            setReceiverArgument(getExtensionReceiverIndex(), value, targetHasExtensionReceiver)
        }
    ...
    private fun setReceiverArgument(index: Int, value: IrExpression?, targetHasThatReceiverParameter: Boolean) {
        if (targetHasThatReceiverParameter) {
            arguments[index] = value
        } else {
            if (value != null) {
                error("Target does not have receiver parameter")
            } else {
                // That's fine
            }
        }
