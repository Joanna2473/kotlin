...
object CommonConfigurationKeysContainer : KeysContainer("org.jetbrains.kotlin.config", "CommonConfigurationKeys") {
    ...
    val LOOKUP_TRACKER by key<LookupTracker>("lookup tracker", allowNull = true)
    val EXPECT_ACTUAL_TRACKER by key<ExpectActualTracker>("expect actual tracker", allowNull = true)
    val INLINE_CONST_TRACKER by key<InlineConstTracker>("inline constant tracker", allowNull = true)
    val ENUM_WHEN_TRACKER by key<EnumWhenTracker>("enum when tracker", allowNull = true)
    val IMPORT_TRACKER by key<ImportTracker>("import tracker", allowNull = true)
