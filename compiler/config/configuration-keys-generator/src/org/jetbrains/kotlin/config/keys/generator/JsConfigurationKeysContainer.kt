...
object JsConfigurationKeysContainer : KeysContainer("org.jetbrains.kotlin.js.config", "JSConfigurationKeys") {
    ...
    val INCREMENTAL_DATA_PROVIDER by key<IncrementalDataProvider>("incremental data provider", allowNull = true)

    val INCREMENTAL_RESULTS_CONSUMER by key<IncrementalResultsConsumer>("incremental results consumer", allowNull = true)

    val INCREMENTAL_NEXT_ROUND_CHECKER by key<IncrementalNextRoundChecker>("incremental compilation next round checker", allowNull = true)
    ...
    val PER_MODULE_OUTPUT_NAME by key<String>("Custom output name to the split .js files", allowNull = true)
    ...
    val CALL_MAIN by key<Boolean>("Specify whether the 'main' function should be called upon execution.")
}
